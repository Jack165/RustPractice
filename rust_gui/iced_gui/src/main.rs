...
pub fn main() {
    Todos::run(Settings {
        default_font: Some(include_bytes!("../fonts/MSYHL.TTC")),
        ..Settings::default()
    });
}
...
const ICONS: Font = Font::External {
    name: "Icons",
    bytes: include_bytes!("../fonts/MSYHL.TTC"),
};
...
// 这个应用可以编译为Web应用，但是这里保存数据在web和普通应用间有点区别，所以这里通过这个宏来描述web编译时用哪个实现，非web应用时用哪种实现
